...
Page({
  ...
  onChooseAvatarAndNickname(e) {
    ...
    this.setData({ 
      ...
      // 延迟触发昵称输入框焦点
      setTimeout(() => {
        that.setData({ focusNickname: true });
      }, 300);
    });
  },
  ...
  onCreateTestGame() {
    ...
    wx.cloud.callFunction({
      ...
      data: {
        type: 'createTestGame',
        gameName: '测试球局-' + new Date().getTime().toString().slice(-4),
        maxPlayers: 8
      },
      success: (res) => {
        wx.hideLoading();
        if (res.result && res.result.success) {
          wx.showToast({ title: '测试球局已创建', icon: 'success' });
          this.loadGameList();
        } else {
  ...
  // 减少场地数量
  onDecreaseCourtCount() {
    const { courtCount } = this.data;
    if (courtCount > 1) {
  ...
  onTouchMove(e) {
    ...
    if (Math.abs(deltaX) > Math.abs(deltaY)) {
      ...
      if (item && item.isCreator) {
        ...
        newTranslateX = Math.max(-150, Math.min(0, newTranslateX));
